...
@testset "print results" begin
    ...
    @test vcov(result)[1] ≈ 3.5384578251636785 atol = 0.01  # Small DOF differences
...
@testset "Predict" begin
    ...
    df = DataFrame(
        x = rand(100), g1 = rand(["a", "b"], 100), g2 = [missing; rand(["c", "d"], 99)],
        g3 = rand(["e", "f"], 100))
    df.y = 1.0 .+ 0.5 .* df.x .+ isequal.(df.g1, "b") .+ isequal.(df.g2, "d") * 2 .+
           isequal.(df.g3, "e")
...
@testset "Continuous/FE detection" begin
    ...
    @test MetricsLinearModels.has_cont_fe_interaction(@formula(y ~ x + y&z)) == false

    # FE/FE interaction also works
    @test MetricsLinearModels.has_cont_fe_interaction(@formula(y ~ x + fe(y)&fe(z))) ==
          false

    # Interaction of FEs with continuous variable requires special handling, currently not implemented
    @test MetricsLinearModels.has_cont_fe_interaction(@formula(y ~ x + fe(y)&z)) == true
    @test MetricsLinearModels.has_cont_fe_interaction(@formula(y ~ x + y&fe(z))) == true
    @test MetricsLinearModels.has_cont_fe_interaction(@formula(y ~ x + fe(y)&fe(z)&a)) ==
          true

    # Interaction of continuous with non-FE function term again handled by StatsModels
    @test MetricsLinearModels.has_cont_fe_interaction(@formula(y ~ x + y^2&z)) == false
end
...
@testset "residuals" begin
    ...
    result = ols(df, model)
    @test residuals(result, df)[1:10] ≈
          [-39.2637, -37.48801, -34.38801, -36.09743, -36.97446,
        -43.15547, -41.22573, -40.83648, -34.52427, -28.91617] atol = 1e-4
    @test r2(result) ≈ 0.0968815737054879 atol = 1e-4
    @test adjr2(result) ≈ 0.0962261902321246 atol = 1e-2  # Small DOF differences
    @test result.nobs == 1380
    @test result.F ≈ 147.8242550248069 atol = 1.0  # Small DOF differences
    ...
    # iv and weights
    model = @formula Sales ~ CPI + (Price ~ Pimin)
    result = iv(TSLS(), df, model, weights = :Pop)
    ...
    @test result.F ≈ 458.4582526109375 atol = 1.0  # Small DOF differences

    # fixed effects and weights
    model = @formula Sales ~ Price + fe(State)
    result = ols(df, model, weights = :Pop, save = true)
    ...
    model = @formula Sales ~ CPI + (Price ~ Pimin) + fe(State)
    result = iv(TSLS(), df, model, save = true)
    ...
    result = ols(df, save = :residuals, weights = :Pop, m)
    ...
    model = @formula Sales ~ Price + fe(State)
    result = ols(df, model, save = :residuals)
    @test residuals(result) !== nothing
    @test "fe_State" ∉ names(fe(result))
...
@testset "saved fixed effects" begin
    ...
    result1 = ols(df, model1, weights = :Pop)
    model2 = @formula Sales ~ Price
    result2 = ols(df, model2, weights = :Pop)
    ...
    model = @formula Sales ~ Price + fe(State)
    result = ols(df, model, subset = df.State .<= 30, save = true)
    ...
    result = iv(TSLS(), df, model, weights = :Pop, save = true)
    ...
    # subset with IV
    model = @formula Sales ~ (Price ~ Pimin) + fe(Year)
    result = iv(TSLS(), df, model, subset = df.State .<= 30, save = true)
    ...
    model = @formula Sales ~ (Price ~ Pimin) + fe(Year)
    result = iv(TSLS(), df, model, subset = df.State .<= 30, weights = :Pop, save = true)
...
@testset "gpu" begin
    ...
    for method in methods_vec
        ...
        local result = ols(
            df, model, save = true, method = method, double_precision = false)
